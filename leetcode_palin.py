...
class Solution:
    ...
    def longestPalindrome(self, s: str) -> str:  # type: ignore
        ...
        self.logger.info(f"Finding longest palindrome in string of length {len(s)}")
        ...
        if len(s) == 1:
            self.logger.info("Single character string - returning as palindrome")
            return s
...
# As nested loops are not efficient for larger strings.
        ...
        self.logger.info("Algorithm completed in %s seconds", time_taken.total_seconds())
        self.logger.info("Found palindrome: '%s' with length %d", longest, len(longest))
